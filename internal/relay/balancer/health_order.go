...
// HealthFirst 健康优先：按通道健康分档（健康/降级/差），档间健康档先，
// 同档内轮换（连续请求不盯死同一通道），档间顺序稳定。
//
...
func (b *HealthFirst) Candidates(items []model.GroupItem) []model.GroupItem {
	...
	for i, item := range items {
		s := channelHealthScore(item.ChannelID, now)
	...
	// 同档内轮换：用全局 roundRobinCounter 给每档一个起始偏移，
	...
	for k := 0; k < n; {
		...
		segLen := end - k
		if segLen > 1 {
			off := int(atomic.AddUint64(&roundRobinCounter, 1) % uint64(segLen))
