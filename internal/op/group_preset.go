...
func groupPresetSnapshotFromCache(groupID int) (mode model.GroupMode, matchRegex string, firstTokenTimeOut, sessionKeepTime, maxRetries int, retryEnabled bool, protocolMode model.ProtocolPolicyMode, preferredProtocols []string, policyRevision int64, items []model.GroupPresetItem, err error) {
	...
	preferredProtocols = append([]string(nil), group.PreferredProtocols...)
	policyRevision = group.PolicyRevision
...
func GroupPresetCreate(groupID int, name string, ctx context.Context) (*model.GroupPreset, error) {
	...
	mode, matchRegex, fto, skt, mr, re, protocolMode, preferredProtocols, policyRevision, items, err := groupPresetSnapshotFromCache(groupID)
	if err != nil {
		return nil, err
	}
...
func GroupPresetClone(presetID int, newName string, ctx context.Context) (*model.GroupPreset, error) {
	...
	clone := model.GroupPreset{
		...
		PreferredProtocols: append([]string(nil), source.PreferredProtocols...),
